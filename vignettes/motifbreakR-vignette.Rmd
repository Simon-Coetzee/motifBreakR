...
1. *Read in Single Nucleotide Variants:* The first step is to read in the list of variants.  Variants can be either a list of rs#'s if your SNPs are represented in one of the SNPlocs packages
2. *Find broken Motifs:* lksdjflkdsj
3. *Visualize SNPs and motifs:* sldkjf




allows representation of the input SNPs as either a list of rs#'s if your input is human, or as a bed file if your snps are not availible in one of the SNPlocs packages or your species is not represented by a SNPlocs package. An example of a bed file can be seen below:
...
Here we can see the full selection of genomes and motifs availble from Bioconductor via `r Biocpkg("BSgenome")` and `r Biocpkg("MotifDb")`
```{r, message=FALSE, cache=TRUE}
library(BSgenome)
available.genomes()
library(MotifDb)
...
Here we can see the additional set of motifs that have been gathered and included in this package:
```{r, message=FALSE, cache=TRUE}
library(motifbreakR)
...
hocomoco
```
See `?motifbreakR` for more information and citations.

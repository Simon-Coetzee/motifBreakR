...
rs2661839 <- results[names(results) %in% "rs2661839"]
rs2661839
```
Here we can see that SNP rs2661839 disrupts multiple motifs. We can then check what the pvalue for each allele is with regard to each motif, using `calculatePvalue`.
```{r calcp, cache=TRUE}
...
## *Step 3* | Visualize
Now that we have our results, we can visualize them with the function `plotMB`.  Lets take a look at rs2661839.
```{r plotting, cache=TRUE, fig.retina=2, fig.align='center', fig.height=8, fig.width=6}
plotMB(results = results, rsid = "rs2661839", effect = "strong")
```
...
`r Githubpkg("Simon-Coetzee/motifBreakR")` does not display neutral effects,
